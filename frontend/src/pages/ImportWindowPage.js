              ...
              {loading ? (
                ...
                getTabImports().map((imp) => {
                  ...
                  return (
                    ...
                    <td className="p-3">
                      <DocumentChecklist 
                        checklist={imp.document_checklist_obj || imp.document_checklist || {}}
...
const DocumentChecklist = ({ checklist, requiredDocs, onUpdate }) => {
  // Handle both object format {key: true} and array format [{type, received}]
  const getDocStatus = (docKey) => {
    if (Array.isArray(checklist)) {
      const doc = checklist.find(d => d.key === docKey || d.type?.toLowerCase().includes(docKey.toLowerCase()));
      return doc?.received || false;
    }
    return checklist[docKey] || false;
  };

  const completedCount = requiredDocs.filter(d => getDocStatus(d.key)).length;
    ...
    <div className="flex items-center gap-2">
      <div className="flex gap-1">
        {requiredDocs.map(doc => {
          const isReceived = getDocStatus(doc.key);
          return (
            <button
              key={doc.key}
              onClick={() => onUpdate(doc.key, !isReceived)}
              className={`p-1 rounded transition-colors hover:bg-muted/30 ${
                isReceived ? 'text-green-400' : 'text-gray-500 hover:text-gray-400'
              }`}
              title={`${doc.label} - Click to ${isReceived ? 'unmark' : 'mark'} as received`}
            >
              {isReceived ? <CheckSquare className="w-4 h-4" /> : <Square className="w-4 h-4" />}
            </button>
          );
        })}
      </div>
      <span className={`text-xs font-medium ${completedCount === totalCount ? 'text-green-400' : 'text-amber-400'}`}>
...
const ImportDetailsModal = ({ importRecord, requiredDocs, onClose, onMoveToTransport, onUpdateDocument }) => {
  return (
        ...
        <div className="space-y-6 py-4">
          ...
          <div>
            <h3 className="font-semibold mb-2 flex items-center gap-2">
              ...
              Required Documents
              <span className="text-xs text-muted-foreground ml-2">(Click to mark as received)</span>
            </h3>
            <div className="grid grid-cols-2 gap-2">
              {requiredDocs.map(doc => {
                // Handle both object and array formats
                const getDocStatus = () => {
                  const checklist = importRecord.document_checklist_obj || importRecord.document_checklist;
                  if (Array.isArray(checklist)) {
                    const docItem = checklist.find(d => d.key === doc.key || d.type?.toLowerCase().includes(doc.key.toLowerCase()));
                    return docItem?.received || false;
                  }
                  return checklist?.[doc.key] || false;
                };
                const isReceived = getDocStatus();
                return (
                  <button
                    key={doc.key}
                    onClick={() => {
                  ...
                  >
                    ...
                    <span className={isReceived ? 'text-green-400' : ''}>{doc.label}</span>
                  </button>
                );
              })}
            </div>
