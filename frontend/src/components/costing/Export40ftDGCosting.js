...
export default function Export40ftDGCosting({ costing, quotation, onUpdate }) {
  const [jobOrder, setJobOrder] = useState(null);
  const [loadingJob, setLoadingJob] = useState(false);
  const [costs, setCosts] = useState({
    transportation_rate: costing?.transportation_rate || 1350.00,
    transportation_units: costing?.transportation_units || 1,
    transportation_total: costing?.transportation_total || 1350.00,
    tluc_rate: costing?.tluc_rate || 314.00,
    tluc_units: costing?.tluc_units || 1,
    tluc_total: costing?.tluc_total || 314.00,
    thc_40ft_dg_rate: costing?.thc_40ft_dg_rate || 1650.00,
    thc_40ft_dg_units: costing?.thc_40ft_dg_units || 1,
    thc_40ft_dg_total: costing?.thc_40ft_dg_total || 1650.00,
    ...
    isps_units: costing?.isps_units || 1,
    isps_total: costing?.isps_total || 110.55,
    seal_charges_rate: costing?.seal_charges_rate || 45.00,
    seal_charges_units: costing?.seal_charges_units || 1,
    seal_charges_total: costing?.seal_charges_total || 45.00,
    ...
    steel_drum_210_new_rate: costing?.steel_drum_210_new_rate || 55.00,
    steel_drum_210_new_units: costing?.steel_drum_210_new_units || 0,
    steel_drum_210_new_total: costing?.steel_drum_210_new_total || 0,
    hdpe_drum_210_reconditioned_rate: costing?.hdpe_drum_210_reconditioned_rate || 39.00,
    ...
    nmc_certificate_rate: costing?.nmc_certificate_rate || 569.63,
    nmc_certificate_units: costing?.nmc_certificate_units || 1,
    nmc_certificate_total: costing?.nmc_certificate_total || 569.63,
    service_fee_rate: costing?.service_fee_rate || 150.00,
    service_fee_units: costing?.service_fee_units || 0,
    service_fee_total: costing?.service_fee_total || 0,
  ...
  const handleChange = (field, value) => {
    ...
    const totalAED = 
      ...
      (newCosts.nmc_certificate_total || 0) +
      (newCosts.service_fee_total || 0) +
  ...
  const charges = [
    { key: 'transportation', label: 'Transportation', field: 'transportation', defaultRate: 1350.00, defaultUnits: 1 },
    { key: 'tluc', label: 'TLUC', field: 'tluc', defaultRate: 314.00, defaultUnits: 1 },
    { key: 'thc_40ft_dg', label: 'THC 40ft DG', field: 'thc_40ft_dg', defaultRate: 1650.00, defaultUnits: 1 },
    ...
    { key: 'isps', label: 'ISPS', field: 'isps', defaultRate: 110.55, defaultUnits: 1 },
    { key: 'seal_charges', label: 'Seal Charges', field: 'seal_charges', defaultRate: 45.00, defaultUnits: 1 },
  ...
  const packagingCharges = [
    ...
    { key: 'steel_drum_210_new', label: 'Steel Drum 210 Ltr-New', field: 'steel_drum_210_new', defaultRate: 55.00 },
    { key: 'hdpe_drum_210_reconditioned', label: 'HDPE Drum 210 Ltr-Reconditioned', field: 'hdpe_drum_210_reconditioned', defaultRate: 39.00 },
    ...
    { key: 'nmc_certificate', label: 'NMC CERTICATE', field: 'nmc_certificate', defaultRate: 569.63, defaultUnits: 1 },
    { key: 'service_fee', label: 'Service Fee', field: 'service_fee', defaultRate: 150.00 },
              ...
              {quotation.items.map((item, itemIdx) => (
                <div key={itemIdx} className="border border-border rounded-lg p-4 space-y-3 bg-muted/10">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <Input
                      type="number"
                      value={(costs[`loaded_weight_mt_${itemIdx}`] || (costs[`drum_ctn_${itemIdx}`] !== undefined ? ((costs[`drum_ctn_${itemIdx}`] || 0) * (costs[`kg_per_drum_ctn_${itemIdx}`] || 0)) / 1000 : (itemIdx === 0 ? (costs.loaded_weight_mt || 0) : 0)))?.toFixed(3) || '0.000'}
                      readOnly
                      className="w-32 text-right bg-muted font-mono"
                    />
                  </div>
