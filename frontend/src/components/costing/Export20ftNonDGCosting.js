...
export default function Export20ftNonDGCosting({ costing, quotation, onUpdate }) {
  const [jobOrder, setJobOrder] = useState(null);
  const [loadingJob, setLoadingJob] = useState(false);
  const [costs, setCosts] = useState({
    transportation_rate: costing?.transportation_rate || 650.00,
    transportation_units: costing?.transportation_units || 1,
    transportation_total: costing?.transportation_total || 650.00,
    tluc_rate: costing?.tluc_rate || 314.00,
    tluc_units: costing?.tluc_units || 1,
    tluc_total: costing?.tluc_total || 314.00,
    thc_20ft_non_dg_rate: costing?.thc_20ft_non_dg_rate || 700.00,
    thc_20ft_non_dg_units: costing?.thc_20ft_non_dg_units || 1,
    thc_20ft_non_dg_total: costing?.thc_20ft_non_dg_total || 700.00,
    ...
    isps_rate: costing?.isps_rate || 49.00,
    ...
    seal_charges_units: costing?.seal_charges_units || 0,
    seal_charges_total: costing?.seal_charges_total || 0,
    ...
    flexi_surcharge_rate: costing?.flexi_surcharge_rate || 52.00,
    flexi_surcharge_units: costing?.flexi_surcharge_units || 0,
    flexi_surcharge_total: costing?.flexi_surcharge_total || 0,
    ...
    steel_drum_210_reconditioned_rate: costing?.steel_drum_210_reconditioned_rate || 44.00,
    ...
    steel_drum_210_new_rate: costing?.steel_drum_210_new_rate || 80.00,
    ...
    flexi_units: costing?.flexi_units || 1,
    flexi_total: costing?.flexi_total || 1010.63,
    pallets_rate: costing?.pallets_rate || 22.00,
    pallets_units: costing?.pallets_units || 0,
    pallets_total: costing?.pallets_total || 0,
    other_charges_rate: costing?.other_charges_rate || 368.50,
    ...
    import_clearance: costing?.import_clearance || 0,
  ...
  const handleChange = (field, value) => {
    ...
    const totalAED = 
      ...
      (newCosts.thc_20ft_non_dg_total || 0) +
      ...
      (newCosts.flexi_surcharge_total || 0) +
    ...
    if (field === 'drum_ctn' || field === 'kg_per_drum_ctn') {
      ...
      newCosts.loaded_weight_mt = (drumCtn * kgPerDrum) / 1000;
    }
    ...
    if (newCosts.loaded_weight_mt > 0) {
      ...
      newCosts.export_shipment_charges_per_mt = newCosts.cost_per_mt;
    } else {
    ...
    // Calculate total cost (Product Cost + Import Clearance + Export Shipment)
    newCosts.total_cost = (newCosts.product_cost || 0) + (newCosts.import_clearance || 0) + (newCosts.export_shipment_charges || 0);
  ...
  const charges = [
    { key: 'transportation', label: 'Transportation', field: 'transportation', defaultRate: 650.00, defaultUnits: 1 },
    { key: 'tluc', label: 'TLUC', field: 'tluc', defaultRate: 314.00, defaultUnits: 1 },
    { key: 'thc_20ft_non_dg', label: 'THC 20ft NON DG', field: 'thc_20ft_non_dg', defaultRate: 700.00, defaultUnits: 1 },
    ...
    { key: 'isps', label: 'ISPS', field: 'isps', defaultRate: 49.00, defaultUnits: 0 },
    ...
    { key: 'seal_charges', label: 'Seal Charges', field: 'seal_charges', defaultRate: 40.00, defaultUnits: 0 },
    ...
    { key: 'flexi_surcharge', label: 'Flexi Surcharge', field: 'flexi_surcharge', defaultRate: 52.00, defaultUnits: 0 },
  ...
  const packagingCharges = [
    { key: 'steel_drum_210_reconditioned', label: 'Steel Drum 210 Ltr-Reconditioned', field: 'steel_drum_210_reconditioned', defaultRate: 44.00 },
    { key: 'steel_drum_210_new', label: 'Steel Drum 210 Ltr-New', field: 'steel_drum_210_new', defaultRate: 80.00 },
    ...
    { key: 'flexi', label: 'Flexi', field: 'flexi', defaultRate: 1010.63, defaultUnits: 1 },
    { key: 'pallets', label: 'Pallets', field: 'pallets', defaultRate: 22.00 },
    { key: 'other_charges', label: 'Other Charges', field: 'other_charges', defaultRate: 368.50 },
  ];
  ...
  return (
    <div className="space-y-6">
                ...
                {packagingCharges.map((charge, index) => (
                  <tr key={charge.key} className="border-b border-border/30">
                    ...
                    <td className="p-2">
                      <Input
                        type="number"
                        value={costs[`${charge.field}_units`] !== undefined ? costs[`${charge.field}_units`] : (charge.defaultUnits !== undefined ? charge.defaultUnits : 0)}
                        onChange={(e) => {
                          const val = e.target.value === '' ? '' : parseFloat(e.target.value);
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex justify-between items-center">
            <Label className="font-semibold">Total cost/ Container ($)</Label>
            <Input
              type="number"
              ...
              {quotation.items.map((item, itemIdx) => (
                <div key={itemIdx} className="border border-border rounded-lg p-4 space-y-3 bg-muted/10">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <Input
                      type="number"
                      value={(costs[`loaded_weight_mt_${itemIdx}`] || (costs[`drum_ctn_${itemIdx}`] !== undefined ? ((costs[`drum_ctn_${itemIdx}`] || 0) * (costs[`kg_per_drum_ctn_${itemIdx}`] || 0)) / 1000 : (itemIdx === 0 ? (costs.loaded_weight_mt || 0) : 0)))?.toFixed(2) || '0.00'}
                      readOnly
                      className="w-32 text-right bg-muted font-mono"
                    />
                  </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex justify-between items-center">
            ...
            <Input
              type="number"
              value={costs.import_clearance || 0}
              onChange={(e) => handleChange('import_clearance', parseFloat(e.target.value) || 0)}
              className="w-32 text-right"
              step="0.01"
            />
          </div>
