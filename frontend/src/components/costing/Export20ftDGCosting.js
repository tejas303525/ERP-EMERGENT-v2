...
export default function Export20ftDGCosting({ costing, quotation, onUpdate }) {
  ...
  const [costs, setCosts] = useState({
    ...
    transportation_total: costing?.transportation_total || 375.00,
    tluc_rate: costing?.tluc_rate || 314.00,
    tluc_units: costing?.tluc_units || 1,
    tluc_total: costing?.tluc_total || 314.00,
    thc_20ft_dg_rate: costing?.thc_20ft_dg_rate || 1050.00,
    thc_20ft_dg_units: costing?.thc_20ft_dg_units || 1,
    thc_20ft_dg_total: costing?.thc_20ft_dg_total || 1050.00,
    ...
    isps_total: costing?.isps_total || 49.00,
    ...
    seal_charges_total: costing?.seal_charges_total || 40.00,
    ...
    do_charges_rate: costing?.do_charges_rate || 0,
    do_charges_units: costing?.do_charges_units || 0,
    do_charges_total: costing?.do_charges_total || 0,
    ...
    nmc_charges_rate: costing?.nmc_charges_rate || 368.50,
    nmc_charges_units: costing?.nmc_charges_units || 0,
    nmc_charges_total: costing?.nmc_charges_total || 0,
  ...
  if (field.startsWith('drum_ctn_') || field.startsWith('kg_per_drum_ctn_')) {
    ...
    // Calculate import shipment charges per MT (but allow manual override field too)
    if (field === 'import_shipment_charges' || field === 'loaded_weight_mt') {
      if (newCosts.loaded_weight_mt > 0) {
        newCosts.import_shipment_charges_per_mt =
          newCosts.import_shipment_charges / newCosts.loaded_weight_mt;
      } else {
        newCosts.import_shipment_charges_per_mt = 0;
      }
    }
  ...
  const charges = [
    { key: 'transportation', label: 'Transportation', field: 'transportation', defaultRate: 375.00, defaultUnits: 1 },
    { key: 'tluc', label: 'TLUC', field: 'tluc', defaultRate: 314.00, defaultUnits: 1 },
    { key: 'thc_20ft_dg', label: 'THC 20ft DG', field: 'thc_20ft_dg', defaultRate: 1050.00, defaultUnits: 1 },
    ...
    { key: 'isps', label: 'ISPS', field: 'isps', defaultRate: 49.00, defaultUnits: 1 },
    ...
    { key: 'seal_charges', label: 'Seal Charges', field: 'seal_charges', defaultRate: 40.00, defaultUnits: 1 },
    ...
    { key: 'do_charges', label: 'DO CHARGES', field: 'do_charges', defaultRate: 0, defaultUnits: 0 },
  ...
  const packagingCharges = [
    ...
    { key: 'nmc_charges', label: 'NMC charges', field: 'nmc_charges', defaultRate: 368.50 },
                ...
                <div key={itemIdx} className="border border-border rounded-lg p-4 space-y-3 bg-muted/10">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <Input
                      ...
                      onChange={(e) => handleChange(`loaded_weight_mt_${itemIdx}`, parseFloat(e.target.value) || 0)}
                      className="w-32 text-right"
                      step="0.001"
                    />
                  </div>
