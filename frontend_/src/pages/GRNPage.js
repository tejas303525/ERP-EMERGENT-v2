...
import { Plus, Receipt, Trash2, Factory, Package } from 'lucide-react';
...
export default function GRNPage() {
  ...
  const loadData = async () => {
    try {
      const [grnsRes, productsRes, productionRes] = await Promise.all([
        grnAPI.getAll(),
        productAPI.getAll(),
        api.get('/grn/production').catch(() => ({ data: [] })),
      ]);
  ...
  const handleCreateProductionGRN = async (job) => {
    try {
      const grnData = {
        ...
        items: [{
          ...
          sku: job.product_sku || job.sku || '-',
          quantity: job.quantity,
          unit: 'KG'
        }],
        notes: `Production completion for ${job.job_number}`
      };
      
      await grnAPI.create(grnData);
      
      // Update job status to ready_for_dispatch
      await api.put(`/job-orders/${job.id}/status`, null, { params: { status: 'ready_for_dispatch' } });
      
      toast.success(`GRN created for ${job.job_number}. Stock updated and job ready for dispatch.`);
      loadData();
    } catch (error) {
      toast.error('Failed to create production GRN');
    }
  };
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="mb-4">
          <TabsTrigger value="inward" className="flex items-center gap-2">
            ...
            Inward Goods ({grns.length})
          </TabsTrigger>
          <TabsTrigger value="production" className="flex items-center gap-2">
            <Factory className="w-4 h-4" />
            Production Completed ({productionJobs.length})
          </TabsTrigger>
        </TabsList>
                  ...
                  {grns.map((grn) => (
                    <tr key={grn.id} data-testid={`grn-row-${grn.grn_number}`}>
                      ...
                      <td className="font-mono font-medium text-green-400">
                        +{grn.items?.reduce((sum, item) => sum + item.quantity, 0).toFixed(2)} KG
                      </td>
        ...
        <TabsContent value="production">
          <div className="data-grid">
            <div className="data-grid-header">
              <h3 className="font-medium">Production Completed Jobs ({productionJobs.length})</h3>
              <p className="text-sm text-muted-foreground">Jobs ready to add to finished goods inventory</p>
            </div>
            {loading ? (
              <div className="p-8 text-center text-muted-foreground">Loading...</div>
              ...
              <div className="empty-state">
                ...
                <p className="empty-state-title">No completed production jobs</p>
                <p className="empty-state-description">Jobs with status "Production_Completed" will appear here</p>
              </div>
            ) : (
              <table className="erp-table w-full">
                <thead>
                ...
                <tbody>
                  {productionJobs.map((job) => (
                    <tr key={job.id}>
                      <td className="font-medium font-mono">{job.job_number}</td>
                      <td>
                        <div>{job.product_name}</div>
                        <div className="text-xs text-muted-foreground">{job.product_sku}</div>
                      </td>
                      <td className="font-mono">{job.quantity} {job.packaging || 'units'}</td>
                      <td className="font-mono">{job.product_current_stock || 0} KG</td>
                      <td>{formatDate(job.production_end)}</td>
                      <td>
                        <Button 
                          size="sm" 
                          onClick={() => handleCreateProductionGRN(job)}
